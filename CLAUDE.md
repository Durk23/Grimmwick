...
- **FROSTMERE (Winterfest expansion — target 1.5, December, FREE per the model lock)**: second TOWN (`08b_frosthub` = hub2 + `FWORLDS` w6–w10; ferry dock in Grimmwick's square, sails post-story), **W6 Glimmerfields BUILT** (`09zn…09zr` via `09zm_w6kit`, enemies `07z`) — 5 levels (Lantern Lane · Frozen Pond · Icicle Grotto · Glimmerglade · Grumble's Doorstep) + **Grumble, the Abominable Snowman** (`10f_boss6`: rebuilds between phases; secret = light the 4 braziers so the snow melts; wholesome pat-him-small ending). New systems: ice physics (`tag:'ice'` colliders + player friction branch), Snow Shoes = 5th Trick (`frostShoes`, grounded-only +10% so comparable-heights holds), Ember Pop throws SNOWBALLS in winter areas (EmberBolt `snow` skin), Mystery Igloo gamble (penguin ambush), Wrong Snowman warp (pound the balls biggest-first), **ICE CROWN** mask (`icecrown`, all 15 w6 stars, `save.iceCrowned`), `winter` music mood, aurora/snowfall ambience. **Difficulty (owner call, Sept 4): BEYOND-D5 mastery-exam band** — post-story audience, 12–15 enemies/level, CP0 + one lit lantern, chained set-pieces (D5 = owner's favorite obstacle style; carry that bar into w7–w10), still Mario-fair. **Scoping law (updated with the ERA II lock, Sept 5)**: the Star Crown's 75 stays scoped to w1–w5 stars (`parseInt(k.slice(1))<=5` filters in both crown computations) and the NIGHTMARE CROWN keeps its heritage 25-level gate via `nightmareTotal(G,true)` — but the LADDERS cover the whole game, and **Winter Nightmare is LIVE** (nightmare law everywhere; `enterLevel` no longer gates w6+). Hearthlight (not embers) is Frostmere's relight meter (`embers` counts w1–w5 only). **ALL FOUR remaining winter districts are BUILT (Sept 5)**: w7 FROZEN LAKE FELL (`09zt…09zx` via `09zs_w7kit`, enemies `07za` — CrackIce panels, Somnambear/Wraithdeer/IceAngler/PolarCub, IcePolisher/IceSaw/RopeTow/MillWheel machines, FisherHut gamble, BellBuoy warp, boss `10g` URSA MAJOR) · w8 THE ICICLE MINES (`09zz+09zza…d` via `09zy_w8kit`, enemies `07zb` — minecarts/bucket-lines/drills/seesaws, Knocker/GemMimic/Rubblekin/CrystalMoth, Glimmering Geode, Off-Limits-Bucket warp, boss `10h` PRISMUS) · w9 EVERGREEN DEEP (`09zzf…j` via `09zze_w9kit`, enemies `07zc` — the GIANT MUSIC BOX, wrecking-baubles, cane swings, updrafts, CarolBoo rounds (`cull=false` law: trio clocks never pause), Advent Stump, Quiet Carol warp, boss `10i` OLD TANNENBAUM) · w10 THE AURORA PALACE (`09zzl…p` via `09zzk_w10kit`, enemies `07zd` — geysers, the Advent Wall, toy soldiers, frost organ (launches set `_springAir` to beat the release-cut), Frost-Locked Chest, Throne's-Shadow stillness warp, finale `10j` THE FIRST FROST — Grimm delivers the invite; Hearthlight relights). Full suite: 50 levels + 10 bosses green.
...
spectral mini-Pip playback on the runT clock; `save.ghostOff` toggle).
✅ THE TWO-LADDER ERA MODEL (owner lock "lock it", Sept 5, commit 32944c8 — supersedes every earlier
board plan): boards are TWO forever, each measuring the WHOLE game, re-minted per expansion era with
fresh GC ids — 🌙 THE NIGHT (`grimmwick.night2`, New Game → BOTH invitations, = `save.longNightT`, raw
centiseconds) · 🌑 THE NIGHTMARE (`grimmwick.nightmare2`, all-50 nightmare best-sum via
`nightmareTotal(G)`; `nightmareTotal(G,true)` keeps the 25-scope that gates the heritage NIGHTMARE
CROWN). The FLAWLESS board is RETIRED (`checkFlawless` no-ops; trick-taint disabled — stars are crown
HUNTS: Star 75 · Ice 15 · Nightmare heritage-25). Flames: 🔥 First Flame = reigning #1 on night2 ·
🖤 Black Flame = #1 on nightmare2. Era I boards (`grimmwick.night/.flawless/.nightmare`) stay in ASC
unattached — honored in-game by the CHAMPIONS' PLINTH in the town square (each era's #1s carved when
the next era begins; resets are coronations). NO per-level/per-district GC boards ever — ghosts are
the per-level competition.
...
**MODEL LOCKED (owner call, Sept 1 2026 — supersedes the pass/cosmetic-SKU plans below): ALL COSMETICS FREE (candy-earnable). The ONLY real-money product is CANDY PACKS (consumable IAP).** No Spook Pass at launch or Halloween; no paid costumes; no paid levels ever. Money buys patience, nothing else. Launch Oct 1 with ZERO purchases ('No purchases' stays in the listing); ~~the Halloween update (~Oct 31) introduces the Candy Shop~~ **TIMING CHANGED (owner call, Sept 2 2026): the Candy Shop ships ASAP with the Tricks** ("so people can have skills built up sooner", quality-first — "take time, whatever will make it land flawlessly"): **Handful $0.99/5,000 🍬 · Bucket $2.99/10,000 🍬 (= one trick) · Cauldron $4.99/40,000 🍬 (= the whole trick quartet)** (owner re-priced Sept 2 2026 — "we need more candy per pack"; supersedes the 1,200/4,000/7,500 draft). ✅ BUILT: `CandyShopPlugin.swift` (StoreKit 2, consumables; ONE grant path via the 'grant' event — inline buys, Ask-to-Buy approvals, and crash-recovered `Transaction.unfinished` all flow through it), JS pipe in 12_main dedupes by txid (`save.iapSeen`, preserved on reset) and skips `candyLifetime` (bought candy never counts as "collected" for board tiebreaks), 🍬 Candy tab in the Cauldron (honest copy, live prices from StoreKit, App-Store-only note on web). Owner ASC prereqs: Paid Applications agreement + banking/tax, then create the 3 consumables (`grimmwick.candy.handful/bucket/cauldron`) and submit WITH the next app version (first IAP must ride a version, like the first leaderboard). Sacred/unbuyable stays: Star Crown, First Flame, playable Grimm, boss skills, Granny Wick. Older pass/costume-pricing text below is historical context only.

**THE TRICKS (owner call, Sept 1 2026): candy-bought PERMANENT fun items — the candy sink that makes candy packs make sense.** ✅ implemented, all four in the shop's Level Ups tab, preserved by resetSave as PROPERTY: 🔥 **Ember Pop** 🍬10,000 (every spin flings a short-range ember projectile, `EmberBolt` in 05, fired via `throwEmber` on the spin edge; `save.emberPop`) · 🦇 **Bat Wings full-time** 🍬10,000 (permanent 4-flutter-jumps + glide; `batT` topped up each frame, rig-safe across costume changes; `save.batWings`) · 🛡️ **Gummy Guard** 🍬10,000 (auto-shield every level, re-forms 25s after a break — `guardT` countdown, fresh at each respawn; `save.gummyGuard`) · 🍭 **Sweet Tooth** 🍬20,000 (all DROPPED candy doubles at the `candyBurst` choke-point — placed star-candy stays 1×; `save.sweetTooth`; double price because it pays for itself, owner call). **Tricks STACK (owner call): buy once, then owned cards become equip/off toggles in the Cauldron** (`save.trickOff[k]`, gameplay gates via the `G.trickOn(k)` helper; equip choices survive resets). **THE FLAWLESS RULE (owner call, Sept 2 2026): an equipped trick taints FLAWLESS NIGHT eligibility** (`save.nightTricked`, cozy-pattern, until `flawlessT` captured; Reset Save = fresh eligible run) — **the First Flame is unbuyable, forever**. Board identities: THE NIGHT = open to everything (Grimm/Zoe/tricks) · FLAWLESS = pure Pip · NIGHTMARE = tricks sealed outright. Rules: tricks are candy-only forever (never direct $); they work everywhere in the main game (consistent with the Grimm/Zoe NG+ board meta) but **NIGHTMARE SEALS ALL BOUGHT TRICKS** (`G.nightmare` gates them; entry toast "Your tricks sleep here.") so the Nightmare leaderboard stays pure skill — timed lantern pickups (incl. 18s wings) still work there for everyone, they're level design not purchases. **Nightmare Mode is ALWAYS unlocked** (owner call, same day: no story-completion gate — `nmToggle` always shown, `G.nightmare = !!nmSel`; normal district/level gates still decide which levels are open).
...
6. **Characters**: ✅ Grimm SHIPPED (1.1): post-story character unlock, SHADOW LEAP double-jump (forward phase burst + 0.45s ghost i-frames; `character:true` in COSTUMES, own shop section, auto-granted at w5 defeat). Zoe SHIPPED (1.1): 20,000-candy character (chars tab), BROOM GLIDE — double-jump keeps standard rise, then holding JUMP while falling floats her at vy≥-2.0 for 1.6s of magic per airtime (`_zoeDJ`/`zoeGlideT`), broom swaps back→under, sparkle trail.
