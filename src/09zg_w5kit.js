...
// gearPlat(G, x, y, opts): a standable brass cog. Static by default (a flat top with a cosmetic spinning gear ring
// under it); pass opts.spin for the visual turn rate. For a MOVING gear, a level uses G.world.addMover with a cog
// mesh factory (deterministic). Standable top = the collider top at y. Returns nothing (adds mesh + collider + ticker).
function gearPlat(G, x, y, opts={}){
  ...
  G.ents.add({ dead:false, cull:false, isEnemy:false, t:rand(0,3), group:new THREE.Group(), update(dt){ this.t+=dt; gear.rotation.z += spin*dt; } });
}
  ...
  open(G){
    ...
    setTimeout(()=>{
      ...
      if(r < 0.08){
        ...
        G.ents.add(new Heart(p.x-1, p.y+1.6, p.z)); G.ents.add(new PowerUp(p.x+1.2, p.y+1.4, p.z, pick(['shield','moon','bat'])));
