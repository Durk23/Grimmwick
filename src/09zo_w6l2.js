...
// MOMENTUM, at D3 "fairly competitive" density (9 enemies, three lanes: ground penguins/snowmen · floating
// boos · diving bats), every threat squawk/stare/squeak-telegraphed and fair:
...
//   BEAT 4 WATCHER'S ISLAND (breather + CP1)   x 86..98   — a snowbank island: grippy relief, a Spooky
//          Snowman that only moves when you don't look, CP1 (lit), and the HIGH ROAD's launch point.
//   BEAT 5 PENGUIN TRAFFIC (ESCALATE)          x 98..114  — penguins toboggan the lane on staggered patrols:
//          their slides read like street traffic — time the hops (a tap always clears them). Overhead, the
//          HIGH ROAD island-hops snow (grip) and ice (slick) platforms with a Heart + a Moon Drop at the end.
//   BEAT 6 THE WRONG SNOWMAN'S ISLAND (CP2)    x 114..130 — somebody built him ALL WRONG, and a lone PURPLE
//          lantern burns where none should — the district's Old Shortcut. Pound his balls big-to-small. CP2.
//   BEAT 7 THE SLALOM (MASTER)                 x 130..164 — everything at once on one racing line: holes on
//          the floor, a penguin crossing between them, a Snow-Boo to freeze as an emergency brake, and two
//          Blizzard Bats diving the air lane. Candy paves the line; skate it like you mean it.
...
// fishing hole costs a heart, never a life). Deterministic to the flake — fixed penguin homes/phases, static
...
function buildW6L2(G){
  ...
  candyLine(G, [[41,0.9,0],[46,0.9,0]], 2);
  candyLine(G, [[51,0.9,0],[54,0.9,0]], 2);
  ...
  // SPOOKY SNOWMAN — stands utterly still while watched, hops closer every moment you look away (eyes flare
  // cyan mid-hop, the tell you can trust). The island's toll: cross it looking over your shoulder.
  G.ents.add(new SpookySnowman(G, 87, 0, 0, {phase:0.0, watchR:9, aggroR:5.5}));   // fleet-audit fix: home 89 hopped to within 0.5u of CP1's respawn — 87 keeps a 2.5u courtesy gap
  G.ents.add(new Checkpoint(95, 0, 1.6, 1));                           // CP1 — LIT, first of the two mid lanterns
  ...
  // TWO penguins on staggered homes/phases: their squawk->slides cross the lane like traffic to time. Homes and
  // ranges are pinned BOTH ways: penguin #2's wake radius (>=x96) never reaches a fresh CP1 respawn at 95, and
  // the worst-case slide (wake at body+5, toboggan 8.9u) tops out at x116.6 — a full clear patch short of the
  // Wrong Snowman's balls at 117.8 (the warp pocket stays a deliberate, safe act).
  G.ents.add(new FrostbitePenguin(G, 103, 0, 0, {phase:0.0, range:2.0, dir:-1}));
  G.ents.add(new FrostbitePenguin(G, 105.5, 0, 0, {phase:0.9, range:2.2, dir:1}));
  ...
  candyLine(G, [[110,0.9,0],[112.5,0.9,0]], 2);                        // low road's own line, under the Heart overhead

  // =============================== BEAT 6 — THE WRONG SNOWMAN'S ISLAND (x 114..130): the Old Shortcut + CP2 ===============================
  ...
  // remembers a shortcut to the festival shore. CLEAR POCKET (owner law): nearest patrol is penguin #3 at
  // 103.3..107.7 (10u clear), worst slide reach 116.6 vs. balls from 117.8, bats live 16u+ downstream — the
  // puzzle is pounded in peace. warpX lands just before the exit gate; full candy bonus, once per run.
  G.ents.add(new WrongSnowman(G, 120, {warpX:172.5, candy:40}));
  G.ents.add(new Checkpoint(127, 0, 1.6, 2));                          // CP2 — LIT, the master beat's mercy
  ...
  signPost(G, 129, 1.7, 0.1, "THE SLALOM. Holes below, penguins crossing, one rude bat overhead. The racing line is paved in candy - skate it like you mean it.");

  // =============================== BEAT 7 — THE SLALOM (x 130..164): MASTER — the racing line ===============================
  w6IceX(G, 130, 164);
  // everything the pond taught, on one line: three holes, a penguin patrolling BETWEEN them (its patrol zone
  // 139.2..142.8 sits exactly in the 138/144 gap — deterministic), a Snow-Boo to freeze as an emergency brake,
  // and two Blizzard Bats on fixed clocks diving the air lane (squeak-telegraphed snapshot dives, never homing).
  ...
  G.ents.add(new SnowBoo(G, 147, 0.5, 0, {phase:0.6, speed:2.0, range:9}));
  w6FishHole(G, 150);
  candyLine(G, [[148.8,0.9,0],[150,1.9,0],[151.2,0.9,0]], 3);
  G.ents.add(new BlizzardBat(G, 154, 5.6, 0, {phase:1.7, range:4, period:3.6, aggroR:5}));
  candyLine(G, [[157,0.9,0],[161,0.9,0]], 2);                          // the sprint out, under bat #2's watch
  ...
  // (owner law): the far shore holds no enemies at all; bat #2's home caps at 158 with aggro reach ~163, and
  // even its documented post-dive drift toward the player leaves 10u+ of calm — opening is a deliberate act,
  // and the penguin ambush pours out onto GRIPPY snow with its 1s spawnGrace.
